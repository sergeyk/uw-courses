...
# Testing this with the Instructor class, but should cover every model extending HasManyEvaluations
describe HasManyEvaluations do
  describe "with all evaluations having all seven keys" do
    before(:each) do
      ...
      @instructor = @eval1.instructor
    end
    
    it "should have all evaluations created" do
      Evaluation.find(:all).size.should == 3
      @instructor.evaluations.size.should == 3
      @instructor.evaluations.include?(@eval1).should be_true
      @instructor.evaluations.include?(@eval2).should be_true
      @instructor.evaluations.include?(@eval3).should be_true
    end
  ...
  describe "with one of two evals missing some keys" do
    ...
    it "should have all evaluations created" do
      ...
      @instructor.evaluations.size.should == 2
      @instructor.evaluations.include?(@eval1).should be_true
      @instructor.evaluations.include?(@eval2).should be_true
    end
  ...
  describe "with only one eval that's missing some keys" do
    ...
    it "should have all evaluations created" do
      ...
      @instructor.evaluations.size.should == 1
      @instructor.evaluations.include?(@eval1).should be_true
    end
  ...
  describe "with no evals" do
    before(:each) do
      @instructor = Instructor.create(:full_name => "THE DUDE")
    end
